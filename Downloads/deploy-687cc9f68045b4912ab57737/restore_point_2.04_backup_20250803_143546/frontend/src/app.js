        ...
        return renderTable('Providers', providers, [
          {
            ...
            render: (item) => (
              <div className="provider-avatar-cell" style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                ...
                {/* Iconița Temporizator - Afișează acțiuni programate */}
                <div style={{ position: 'relative' }}>
                  <button
                    ...
                    onClick={() => {
                      // Deschide modal cu acțiuni programate pentru acest provider
                      const providerScheduledChanges = scheduledChanges.filter(change => 
                        change.entityType === 'providers' && change.itemId === item.id
                      );
                      if (providerScheduledChanges.length > 0) {
                    ...
                    if (providerScheduledChanges.length > 0) {
                      return (
                        <div style={{
                          ...
                          {providerScheduledChanges.length}
                        </div>
                      );
                    }
                    return null;
                ...
                <div style={{ position: 'relative' }}>
                  <button
                    ...
                    onClick={() => {
                      // Deschide modal cu istoric modificări pentru acest provider
                      const providerHistory = changeHistory.filter(entry => 
                        entry.entityType === 'providers' && entry.itemId === item.id
                      );
