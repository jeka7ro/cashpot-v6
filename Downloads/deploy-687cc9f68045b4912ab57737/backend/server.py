...
async def get_cabinets(current_user: User = Depends(get_current_user)):
    query = await filter_by_user_access(current_user, {}, "cabinets")
    cabinets = await db.cabinets.find(query).to_list(1000)
    # Convert ObjectIds to strings
...
async def update_cabinet(cabinet_id: str, cabinet_data: CabinetCreate, current_user: User = Depends(get_current_user)):
    if current_user.role not in [UserRole.ADMIN, UserRole.MANAGER]:
        raise HTTPException(status_code=403, detail="Insufficient permissions")
    ...
    update_data = cabinet_data.dict()
    await db.cabinets.update_one({"id": cabinet_id}, {"$set": update_data})
    
    updated_cabinet = await db.cabinets.find_one({"id": cabinet_id})
...
async def delete_cabinet(cabinet_id: str, current_user: User = Depends(get_current_user)):
    ...
    # HARD DELETE - actually remove from database
    await db.cabinets.delete_one({"id": cabinet_id})
...
async def get_entity_attachments(entity_type: str, entity_id: str, current_user: User = Depends(get_current_user)):
    ...
    # Convert ObjectIds to strings
    attachments = [convert_objectid_to_str(attachment) for attachment in attachments]
...
async def get_metrology(current_user: User = Depends(get_current_user)):
    try:
        ...
        for item in metrology_list:
            try:
                converted_item = convert_objectid_to_str(item)
                # Skip items with missing required fields
                if all(field in converted_item for field in ['expiry_date', 'calibration_interval', 'next_calibration_date']):
                    valid_metrology.append(Metrology(**converted_item))
            except Exception as e:
                print(f"Error processing metrology item: {e}")
                continue
        return valid_metrology
    except Exception as e:
        print(f"Error in get_metrology: {e}")
        return []
    ...
    try:
        ...
        for item in jackpot_list:
            try:
                converted_item = convert_objectid_to_str(item)
                # Skip items with missing required fields
                if all(field in converted_item for field in ['serial_number', 'current_amount', 'max_amount', 'reset_amount', 'increment_rate', 'last_reset_date', 'description']):
                    valid_jackpots.append(Jackpot(**converted_item))
            except Exception as e:
                print(f"Error processing jackpot item: {e}")
                continue
        return valid_jackpots
    except Exception as e:
        print(f"Error in get_jackpots: {e}")
        return []
