...
export const Company = {
  ...
  async update(id, companyData) {
    console.log('Company.update() called for ID:', id, 'with:', companyData);
    ...
    if (index !== -1) {
      localCompanies[index] = updatedCompany;
      localStorage.setItem('companies', JSON.stringify(localCompanies));
    }
  ...
  async delete(id) {
    console.log('Company.delete() called for ID:', id);
    const localData = localStorage.getItem('companies');
    const localCompanies = localData ? JSON.parse(localData) : [];
    const index = localCompanies.findIndex(comp => comp.id === id);
    if (index !== -1) {
  ...
  async get(id) {
    console.log('Company.get() called for ID:', id);
    const companies = await this.list();
    return companies.find(company => company.id === id) || null;
  }
};

// Location entity using real Google Sheets data
export const Location = {
  async list() {
    try {
      console.log('Location.list() called - combining Google Sheets with localStorage');
      ...
      const googleLocations = getUniqueLocations(slotMachines);
      ...
      const allLocations = [...googleLocations];
      
      // Override with localStorage data (local changes take precedence)
      localLocations.forEach(localLoc => {
        const existingIndex = allLocations.findIndex(loc => loc.name === localLoc.name);
        if (existingIndex !== -1) {
          allLocations[existingIndex] = localLoc;
        } else {
          allLocations.push(localLoc);
        }
      });
      
      console.log(`Loaded ${googleLocations.length} locations from Google Sheets and ${localLocations.length} from localStorage`);
      return allLocations;
    } catch (error) {
  ...
  async create(locationData) {
    console.log('Location.create() called with:', locationData);
    const newLocation = {
      id: `loc-${Date.now()}`,
      ...locationData,
      created_date: new Date().toISOString(),
  ...
  async update(id, locationData) {
    console.log('Location.update() called for ID:', id, 'with:', locationData);
    ...
    if (index !== -1) {
      localLocations[index] = updatedLocation;
      localStorage.setItem('locations', JSON.stringify(localLocations));
    }
  ...
  async delete(id) {
    console.log('Location.delete() called for ID:', id);
    const localData = localStorage.getItem('locations');
    const localLocations = localData ? JSON.parse(localData) : [];
    const index = localLocations.findIndex(loc => loc.id === id);
    if (index !== -1) {
  ...
  async get(id) {
    console.log('Location.get() called for ID:', id);
    const locations = await this.list();
    return locations.find(location => location.id === id) || null;
  }
};

// Provider entity combining Google Sheets data with localStorage
export const Provider = {
  async list() {
    try {
      console.log('Provider.list() called - combining Google Sheets with localStorage');
      // Get data from Google Sheets first
      const sheetData = await getGoogleSheetsData('SlotMachines!A:E');
      const slotMachines = parseSlotMachineData(sheetData);
      const googleProviders = getUniqueManufacturers(slotMachines);
      ...
      // Combine both sources, with local data taking precedence
      const allProviders = [...googleProviders];
      
      // Override with localStorage data (local changes take precedence)
      localProviders.forEach(localProv => {
        const existingIndex = allProviders.findIndex(prov => prov.name === localProv.name);
        if (existingIndex !== -1) {
          allProviders[existingIndex] = localProv;
        } else {
          allProviders.push(localProv);
        }
      });
      
      console.log(`Loaded ${googleProviders.length} providers from Google Sheets and ${localProviders.length} from localStorage`);
      return allProviders;
    } catch (error) {
  ...
  async create(providerData) {
    console.log('Provider.create() called with:', providerData);
    const newProvider = {
      id: `prov-${Date.now()}`,
      ...providerData,
      created_date: new Date().toISOString(),
    ...
    console.log('Provider saved to localStorage:', newProvider);
    console.log('All providers in localStorage:', localProviders);
    console.log('localStorage providers count:', localProviders.length);
    
    return newProvider;
  },

  async update(id, providerData) {
    console.log('Provider.update() called for ID:', id, 'with:', providerData);
    ...
    if (index !== -1) {
      localProviders[index] = updatedProvider;
      localStorage.setItem('providers', JSON.stringify(localProviders));
      console.log('Provider updated in localStorage:', updatedProvider);
      console.log('All providers in localStorage:', localProviders);
    } else {
      console.log('Provider not found in localStorage, adding new one');
      localProviders.push(updatedProvider);
      localStorage.setItem('providers', JSON.stringify(localProviders));
    }
  ...
  async delete(id) {
    console.log('Provider.delete() called for ID:', id);
    const localData = localStorage.getItem('providers');
    const localProviders = localData ? JSON.parse(localData) : [];
    const index = localProviders.findIndex(prov => prov.id === id);
    if (index !== -1) {
  ...
  async get(id) {
    console.log('Provider.get() called for ID:', id);
